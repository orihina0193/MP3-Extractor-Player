...
export default function Extractor({ onRefresh }: ExtractorProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Auto sync to GitHub if enabled and configured
      ...
      const ghStatusMsg = ghConfig.autoSync && isGitHubConfigured(ghConfig)
        ? "ブラウザとGitHubリポジトリの両方に正常保存されました！"
        : "ブラウザキャッシュに保存されました！「再生モード」でいつでも再生可能です。";
      alert(ghStatusMsg);
