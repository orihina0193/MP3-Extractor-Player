...
import { Download, Upload, Trash2, AlertTriangle, Loader2, RefreshCw, FileAudio, Github, HardDrive } from "lucide-react";
...
export default function BackupRestore({ tracks = [], onRefresh }: BackupRestoreProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white/5 border border-white/10 rounded-2xl p-2">
        <div className="flex gap-2 w-full sm:w-auto">
          <button
            onClick={() => setActiveTab("github")}
            className={`flex-1 sm:flex-initial flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold font-mono transition cursor-pointer ${
              activeTab === "github"
          ...
          >
            ...
            <span>GitHub クラウド同期 (1曲ずつ個別)</span>
          </button>
          <button
          ...
          >
            ...
            <span>一括 ZIP バックアップ / 復元</span>
          </button>
        </div>
      </div>

      {activeTab === "github" ? (
        <GitHubSync tracks={tracks} onRefresh={onRefresh} />
      ) : (
        <div className="bg-white/5 border border-white/10 rounded-3xl p-6 sm:p-8 space-y-6">
          <div className="flex flex-col gap-1">
            <h3 className="text-sm font-bold text-[#FF5F1F] tracking-widest uppercase flex items-center gap-2">
              <HardDrive className="w-4 h-4 text-[#FF5F1F]" />
              <span>ローカル ZIP バックアップとデータ管理</span>
            </h3>
            <p className="text-xs text-slate-400 font-mono">
              いつでもお使いの全音楽データをZIPファイルで一括エクスポート・インポートできます。
            </p>
          </div>
