...
import { Music, Youtube, Settings, Disc, Volume2, Database, HelpCircle, Github } from "lucide-react";
        ...
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`flex items-center gap-2 px-3 py-2 rounded-xl border transition cursor-pointer text-xs font-bold ${
                showGithubSettings
                  ? "bg-[#FF5F1F]/20 border-[#FF5F1F]/40 text-[#FF5F1F] glow-orange"
                  : "bg-white/5 border-white/10 text-slate-300 hover:text-[#FF5F1F] hover:bg-white/10"
              }`}
              title="GitHub クラウドストレージ設定"
            >
              <Github className="w-4 h-4 text-[#FF5F1F]" />
              <span className="hidden md:inline">GitHub同期</span>
            </button>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-8 py-8 space-y-8">
        ...
        {isLoading ? (
          ...
          <>
            ...
            {/* GitHub Cloud Storage Panel */}
            {showGithubSettings && (
              <div className="max-w-3xl mx-auto mt-6 pt-4 border-t border-white/10 animate-fade-in">
                <GitHubSettings onRefresh={fetchTracks} />
              </div>
            )}
            ...
            {showSettings && (
              <div className="max-w-3xl mx-auto mt-6 pt-4 border-t border-white/10 animate-fade-in">
                <BackupRestore onRefresh={fetchTracks} />
              </div>
            )}
          </>
        )}
